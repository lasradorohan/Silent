...
class ListSingleHeaderDecoration(context: Context?, private val headerText: String): RecyclerView.ItemDecoration() {
    ...
    private val headerTextBaselineHeight: Int
    private val headerTextBottom: Int
    private val headerTextPaddingVertical = context?.resources?.dpToPx(8) ?: 0
    init {
        val rect = Rect()
    ...
    override fun getItemOffsets(
        ...
        if (position==0){
            outRect.top = headerTextBaselineHeight + 2*headerTextPaddingVertical
        }
    }
...
//                c.drawRect(Rect(view.left, view.top - rectHeight, view.right, view.top), paint)
                c.drawText(headerText,
                    view.left.toFloat(), view.top-headerTextBottom.toFloat()-headerTextPaddingVertical, headerTextPaint)
